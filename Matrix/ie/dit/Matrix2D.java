...
public class Matrix2D {
	...
	public static Matrix2D add(Matrix2D a, Matrix2D b) {
		...
		if(a.rows != b.rows || a.cols != b.cols) {
			...
			return c;	// This would be an exception
	...
	public void setCell(int row, int col, float val) { //throws Matrix2DException {
	...
	public void addCell(int row, int col, float val) {
	...
	public void randomise(float max) {
		...
		for(int row = 0; row < rows; row++) {
			...
			for(int col = 0; col < cols; col++) {
				...
				setCell(row, col, max * new Random().nextFloat());
			}
		}
